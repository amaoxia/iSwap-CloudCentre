...
<div class="frameset_w" style="height:100%;background-color:#FFFFFF;">
  <div class="main"><div>
   <form name="save" id="saveForm" action="${path}/cloudnode/workflow/workflow!add.action?deptId=${deptId?default('')}" method="post">
	<@s.token/>
	<input type="hidden" id="deptId" name="deptId" value="${deptId?default('')}"/>
				...
				<ul class="item1_c">
					...
					<li class="item2_bg">
					 ...
					 <li>
					  <p><b>*</b>流程类型：</p>
					  <span>
					  <input name="wfType" type="radio" value="1" checked="checked"/>
	                    	  发送&nbsp;&nbsp;
	                      <input name="wfType" type="radio" value="0"/>
	                      	接收
	                       </span>
					</li>
					  <li class="item2_bg">
					  <p><b>*</b>所属应用：</p>
					  <span>
					  <select name="appMsg.id" id="appMsg"  style="width:200px">
						<option value="">--请选择--</option>
						<#list appMsgList as app>
						<option value="${app.id}">${app.appName?default('')}</option>
						</#list>
						</select>
					  </span>
					...
					<li id="dsType" name="dsType" class="">
					  ...
					  -->
					  <#-- 
					   <input name="dataType" type="radio" value="0" checked="checked" />
                      	数据库数据&nbsp;&nbsp;
...
<script type='text/javascript'>
	$(function(){ 
	 ...
	 $("input[type='radio'][name='dataType']:first").trigger("click");
</script>
</body>
</html>
