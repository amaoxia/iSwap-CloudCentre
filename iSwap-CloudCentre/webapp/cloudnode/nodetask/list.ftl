			      ...
			      <table class="tabs1"  style="margin-top:0px;">
			        ...
			        <#list listDatas as entity>
			         <#assign i = i+1>
			          ...
			          <td>
					  	<a href="javascript:void(0);" <#if (entity.status?exists&&entity.status=='1')>style="color:#ccc;"</#if> <#if (entity.id?exists)>onclick="opdg('${path}/cloudnode/nodetask/task!updateView.action?deptId=${deptId?default('')}&id=${entity.id?default('')}','定制流程任务调度','620','560');" <#else>onclick="opdg('${path}/cloudnode/nodetask/task!addView.action?workFlowId=${entity.workFlow.id}','定制流程任务调度','620','560');" </#if>class="tabs1_cz"><img src="${path}/images/czimg_edit.gif" />定制任务</a> 
					  	<#if entity.id?exists>
