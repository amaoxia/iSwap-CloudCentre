...
<div class="frameset_w" style="height:100%">
  <div class="main">
    <div class="common_menu">
      <div class="c_m_title"><img src="${path}/images/title/ftpAccessRuleSet.png"  align="absmiddle" />客户端访问规则设置</div>
      <div class="c_m_btn" style="margin-top:34px;">
        <div class="c_m_btn"> 
        	<span class="cm_btn_m">
        		<a href="javascript:void(0)" id="hz1" onclick="addFtpAccessRule();"><b>
        			<img src="${path}/images/cmb_xj.gif" class="cmb_img" />
        			新建客户端访问规则<a href="javascript:void(0)">
...
<script type="text/javascript" class="source"> 
	...
	$(function () {
     ...
     .bind('loaded.jstree', function(event){
     	if(selectedId+"t"!="t"){
     		$("#content").attr("src","ftpClient!ftpAccessRuleList.action?id="+selectedId);
     	}
	  })
		.bind('click.jstree', function(event){
			...
			if (eventNodeName == 'INS') {
				...
				if($city.parent().attr('rel')==''){
			 		$("#content").attr("src","ftpClient!ftpAccessRuleList.action?id="+id);
			    }
			}
		})
		.jstree({
     ...
     function addFtpAccessRule(){
     	var id=$("#content").contents().find("#ftpUserId").val();
     	showWindow('${path}/ftp/ftpClient/ftpClient!addFtpAccessRule.action?id='+id,'IP访问规则添加',620,370);
     }
