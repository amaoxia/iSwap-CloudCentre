...
<body>	
<form action="${path}/ftp/ftpClient/ftpClient!updateFtpAccessRule.action" method="post" name="saveForm" id="saveForm">
<input type="hidden" name="accessRuleId" value="${ftpAccessRule.id}"/>
              ...
              <div class="item1">
                  <ul class="item1_c">
                    <li>
                      <p>规则：</p>
                      <span>
                      <input type="text" size="30" name="ftpAccessRule.ip" value="${ftpAccessRule.ip}" />
                      </span>
                      <div class="item_ts"><img src="${path}/images/right.png" align="absmiddle"/> 不超过25个汉字，或不超过50个字节（字母，数字和下划线）</div>
                    </li>
					<li class="item_bg">
                      <p>访问情况：</p>
                     <span>
                      <select name="ftpAccessRule.enableaccess" id="select">
                      <#if ftpAccessRule.enableaccess==true>
