...
<div class="frameset_w" style="height:100%">
  <div class="main">
    <div class="common_menu">
      <div class="c_m_title"><img src="${path}/images/title/transmissionLimitSet.png"  align="absmiddle" />上传下载管理</div>
    </div>
    <div class="main_c2" >
...
<script type="text/javascript" class="source"> 
	...
	$(function () {
     ...
     .bind('loaded.jstree', function(event){
     	if(selectedId+"t"!="t"){
     		$("#content").attr("src","ftpClient!editTransmissionLimit.action?id="+selectedId);
     	}
	  })
		.bind('click.jstree', function(event){
			...
			if (eventNodeName == 'INS') {
				...
				if($city.parent().attr('rel')==''){
			 		$("#content").attr("src","ftpClient!editTransmissionLimit.action?id="+id);
			    }
			}
		})
		.jstree({
