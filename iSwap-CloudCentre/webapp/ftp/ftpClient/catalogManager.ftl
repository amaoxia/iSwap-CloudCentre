...
<input type="hidden" id="ftpUserId" value="${selectedId}"/>
<input type="hidden" name="id" value="${selectedId}"/>
  <div class="main">
    <div class="common_menu">
      <div class="c_m_title"><img src="${path}/images/title/catalogManager.png"  align="absmiddle" />目录管理</div>
            <div class="c_m_btn" style="margin-top:34px;">
        <div class="c_m_btn"> 
        	<span class="cm_btn_m">
        		<a href="javascript:void(0)" id="hz1" onclick="addCatalog();"><b>
        			<img src="${path}/images/cmb_xj.gif" class="cmb_img" />
        			新建路径名称<a href="javascript:void(0)">
...
<script type="text/javascript" class="source"> 
	...
	$(function () {
     ...
     .bind('loaded.jstree', function(event){
     	if(selectedId+"t"!="t"){
     		$("#content").attr("src","ftpClient!listCatalog.action?userId="+selectedId);
     	}
	  })
		.bind('click.jstree', function(event){
			...
			if (eventNodeName == 'INS') {
				...
				if($city.parent().attr('rel')==''){
			 		$("#content").attr("src","ftpClient!listCatalog.action?userId="+id);
			    }
			}
		})
		.jstree({
    ...
    function addCatalog(){
     	...
     	showWindow('${path}/ftp/ftpClient/ftpClient!addCatalog.action?id='+id,'路径名称添加',620,190);
     }
     
     function reloadPage(id){
     	$("#content").attr("src","${path}/ftp/ftpClient/ftpClient!listCatalog.action?userId="+id);
     }
</script>
</body>
</html>
