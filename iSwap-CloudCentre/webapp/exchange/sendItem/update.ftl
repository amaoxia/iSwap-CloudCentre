...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<form method="post" action="${path}/exchange/sendItem/sendItem!update.action" id="saveForm">
		    <@s.token/>
		    <input type="hidden"  name="id" value="${id}"/>
		    <input type="hidden"  name="changeItemCycle.id" value="<#if changeItemCycle?exists>${changeItemCycle.id}</#if>"/>
		    <input type="hidden"  name="itemType" value="1"/>
		    <input type="hidden" id="deptId" value="${appItemExchangeConf.sendDept.id}"/> 
		          ...
		          <table width="100%" border="0" cellspacing="0" cellpadding="0"  height="100%">
		            <tr>
		              ...
		              <div class="item1">
		                    ...
		                    <li >
		                      ...
		                      <span id="dataTypeSpan">
			                      <select name="dataType" id="dataType" onchange="changeData(this)" style="width:200px">
				                      ...
				                      <option value="0" <#if dataType?exists&&dataType=='0'>selected="selected"</#if>>文档</option>
				                      <option value="1" <#if dataType?exists&&dataType=='1'>selected="selected"</#if>>数据库</option>
			                      </select>
		                      </span>
		                 	...
		                 	<li>
		                      <span>
		                                                          文档结构类型
			                      <select name="dataStructure" id="dataStructureSel" onchange="addItem(this)" <#if dataType?exists&&dataType=='1'>disabled="true"</#if>>
				                      <option value="">请选择...</option>
		                  ...
		                  <li id="dsTypeLi" name="dsType" class="item_bg" defaultValue="${dsType?default('')}">
							  <p><b>*</b>数据源类型：</p>
							  <span>
						  ...
						  <li> 
							  <p><b>*</b>数据源：</p>
							  <span>
							  <select name="datSource.id" id="dataSourceSel" style="width:200px" defaultValue="<#if datSource?exists>${datSource.id}</#if>">
							  </select>
							  </span>
		                  ...
		                  <li >
		                      ...
		                      <span>
		                        <select name="cycle.exchangeCycleValue" id="exchangeCycleValue"  style="width:200px" onchange="linkageCycle(parseInt(this.value))">
			                     	<option value="0" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="0">selected</#if>>周</option>
				                    <option value="1" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="1">selected</#if>>月</option>
				                    <option value="2" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2">selected</#if>>季</option>
				                    <option value="3" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="3">selected</#if>>年</option>
		                   	    </select> 
							</span>
		                  </li>
		                   <#assign cycleArr="">
		                   <#if changeItemCycle?exists>
		                   	<#assign cycleArr=(changeItemCycle.exchangeDateRule?split(","))>
		                   </#if>
		                  <li  class="item_bg">
		                      <p>交换周期规则:</p>
			                      <span id="w_span" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue!="0">style="display: none;"</#if>>
			                     		 本周数据在:
			                       <select id="w_sel" name="w_sel" style="width:80px">
				                      <option value="0" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="0"&&cycleArr[0]=="0">selected</#if>>
				                     	 本周
				                      </option>
				                      <option value="1" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="0"&&cycleArr[0]=="1">selected</#if>>
				                  	    下周
				                      </option>
				                  </select>
				                      <select id="w_day" name="w_day" style="width:60px">
					                     <#list   1..7 as temp>
				   						  <option value="${temp}" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="0"&&cycleArr[1]==(temp+"")>selected</#if>>${temp}</option>
										 </#list>
			                     	 </select>
			                     	 	 执行
			                      </span>
			                      <!--- 月 --->
		                        <span  style="" id="m_span" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="1"><#else>style="display: none;"</#if>>
		                  	  	  本月数据在:
		                         <select id="m_sel" name="m_sel" style="width:80px">
				                     <option value="0" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="1"&&cycleArr[0]=="0">selected</#if>>
				                     	 本月
				                      </option>
				                      <option value="1" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="1"&&cycleArr[0]=="1">selected</#if>>
				                  	    下月
				                      </option>
			                      </select>
			                      <select id="m_day" name="m_day" style="width:60px">
			                     	<#list 1..31 as temp>
		   						  	<option value="${temp}" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="1"&&cycleArr[1]==(temp+"")>selected</#if>>${temp}</option>
									 </#list>
		                     	 </select>
		                     	 执行
		                      </span>
		                      <!--- 季 -->
		                        <span id="j_span" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"><#else>style="display: none;"</#if>>
		                     	   本季数据在:
		                          <select id="j_sel" name="j_sel" style="width:80px">
			                      <option value="0" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"&&cycleArr[0]=="0">selected</#if>>本季</option>
			                      <option value="1" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"&&cycleArr[0]=="1">selected</#if>>下季    </option>
			                      </select>
			                      <select id="j_month" name="j_month" style="width:60px">
		   						  <option value="1" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"&&cycleArr[1]=="1">selected</#if>>第一月</option>
		   						  <option value="2" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"&&cycleArr[1]=="2">selected</#if>>第二月</option>
		   						  <option value="3" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"&&cycleArr[1]=="3">selected</#if>>第三月</option>
		                     	 </select>
		                     	 的第
		                     	 <select id="j_day" name="j_day" style="width:60px">
			                     <#list   1..31 as temp>
		   						  <option value="${temp}" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="2"&&cycleArr[2]==(temp+"")>selected</#if>>${temp}</option>
								 </#list>
		                     	 </select>
		                     	 日执行
		                      </span>
		                      <!--- 年 -->
		                         <span id="y_span" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="3"><#else>style="display: none;"</#if>>
		                       	  本年数据在:
		                          <select id="y_sel" name="y_sel" style="width:80px">
			                    <option value="0" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="3"&&cycleArr[0]=="0">selected</#if>>
			                     	 本年
			                      </option>
			                      <option value="1" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="3"&&cycleArr[0]=="1">selected</#if>>
			                  	          下年
			                      </option>
			                      </select>
			                      <select id="y_month" name="y_month" style="width:60px">
			                       <#list 1..12 as temp>
								 	<option value="${temp}" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="3"&&cycleArr[1]==(temp+"")>selected</#if>>${temp}</option>
								 </#list>
		                     	 </select>
		                     	 月
		                     	 <select id="y_day" name="y_day" style="width:60px">
			                     <#list 1..31 as temp>
		   						  <option value="${temp}" <#if changeItemCycle?exists&&changeItemCycle.exchangeCycleValue=="3"&&cycleArr[2]==(temp+"")>selected</#if>>${temp}</option>
								 </#list>
		                     	 </select>
		                     	 日执行
		                      </span>
			...
			$("input[type='radio'][name='dsType']").click(function(){
				...
				if(url){
					$.post(url,{deptId:deptId},function(result, status){
						...
						$('#dataSourceSel option[value="'+$('#dataSourceSel').attr("defaultValue")+'"]').attr("selected",true);
					});
				}
			});
		    $("input[type='radio'][name='dsType'][value='"+$('#dsTypeLi').attr("defaultValue")+"']").trigger("click");
		});
	</script>
	</body>
</html>