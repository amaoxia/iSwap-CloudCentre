...
<html xmlns="http://www.w3.org/1999/xhtml">
		              ...
		              <div class="item1">
		                  <ul class="item1_c">
		                    ...
		                    <li class="item_bg">
		                      <p>指标编码:</p>
		                      <span>
		                      <input type="text" name="itemCodee" id="itemCodee" size="30" value="${itemCode?default('')}" disabled="disabled"/></span>
						  ...
						  <li> 
							  <p><b>*</b>数据源：</p>
							  <span>
							  <select name="dataSource.id" id="dataSourceSel" style="width:200px" >
							  </select>
							  </span>
		                  ...
		                  <li >
		                      ...
		                      <span>
		                        <select name="cycle.exchangeCycleValue" id="exchangeCycleValue" style="width:200px" onchange="linkageCycle(parseInt(this.value))">
				                    <option value="0" <#if changeItemCycle.exchangeCycleValue=="0">selected</#if>>周</option>
				                    <option value="1" <#if changeItemCycle.exchangeCycleValue=="1">selected</#if>>月</option>
				                    <option value="2" <#if changeItemCycle.exchangeCycleValue=="2">selected</#if>>季</option>
				                    <option value="3" <#if changeItemCycle.exchangeCycleValue=="3">selected</#if>>年</option>
		                   	    </select> 
							</span>
		                  </li>
		                  ...
		                  <li class="item_bg">
		                      <p>交换日期规则:</p>
		                      <!--<span id="w_span" <#if changeItemCycle.exchangeCycleValue!="0">style="display: none;"</#if>>
		                     		 本周数据在:
		                       <select id="w_sel" name="w_sel" style="width:80px">
			                      <option value="0" <#if changeItemCycle.exchangeCycleValue=="0"&&cycleArr[0]=="0">selected</#if>>
			                     	 本周
			                      </option>
			                      <option value="1" <#if changeItemCycle.exchangeCycleValue=="0"&&cycleArr[0]=="1">selected</#if>>
			                  	    下周
			                      </option>
			                  </select>
			                      <select id="w_day" name="w_day" style="width:60px">
				                     <#list   1..7 as temp>
			   						  <option value="${temp}" <#if changeItemCycle.exchangeCycleValue=="0"&&cycleArr[1]==(temp+"")>selected</#if>>${temp}</option>
									 </#list>
		                     	 </select>
		                     	 	 执行
		                      </span>-->
		                      <!--- 月 --->
		                        <span  style="" id="m_span" <#if changeItemCycle.exchangeCycleValue!="1">style="display: none;"</#if>>
		                  	  	  本月数据在:
		                         <select id="m_sel" name="m_sel" style="width:80px">
				                     <option value="0" <#if changeItemCycle.exchangeCycleValue=="1"&&cycleArr[0]=="0">selected</#if>>
				                     	 本月
				                      </option>
				                      <option value="1" <#if changeItemCycle.exchangeCycleValue=="1"&&cycleArr[0]=="1">selected</#if>>
				                  	    下月
				                      </option>
			                      </select>
			                      <select id="m_day" name="m_day" style="width:60px">
			                     	<#list 1..31 as temp>
		   						  	<option value="${temp}" <#if changeItemCycle.exchangeCycleValue=="1"&&cycleArr[1]==(temp+"")>selected</#if>>${temp}</option>
									 </#list>
		                     	 </select>
		                     	 执行
		                      </span>
		                      <!--- 季 -->
		                        <span id="j_span" <#if changeItemCycle.exchangeCycleValue!="2">style="display: none;"</#if>>
		                     	   本季数据在:
		                          <select id="j_sel" name="j_sel" style="width:80px">
			                      <option value="0" <#if changeItemCycle.exchangeCycleValue=="2"&&cycleArr[0]=="0">selected</#if>>本季</option>
			                      <option value="1" <#if changeItemCycle.exchangeCycleValue=="2"&&cycleArr[0]=="1">selected</#if>>下季    </option>
			                      </select>
			                      <select id="j_month" name="j_month" style="width:60px">
		   						  <option value="1" <#if changeItemCycle.exchangeCycleValue=="2"&&cycleArr[1]=="1">selected</#if>>第一月</option>
		   						  <option value="2" <#if changeItemCycle.exchangeCycleValue=="2"&&cycleArr[1]=="2">selected</#if>>第二月</option>
		   						  <option value="3" <#if changeItemCycle.exchangeCycleValue=="2"&&cycleArr[1]=="3">selected</#if>>第三月</option>
		                     	 </select>
		                     	 的第
		                     	 <select id="j_day" name="j_day" style="width:60px">
			                     <#list   1..31 as temp>
		   						  <option value="${temp}" <#if changeItemCycle.exchangeCycleValue=="2"&&cycleArr[2]==(temp+"")>selected</#if>>${temp}</option>
								 </#list>
		                     	 </select>
		                     	 日执行
		                      </span>
		                      <!--- 年 -->
		                         <span id="y_span" <#if changeItemCycle.exchangeCycleValue!="3">style="display: none;"</#if>>
		                       	  本年数据在:
		                          <select id="y_sel" name="y_sel" style="width:80px">
			                    <option value="0" <#if changeItemCycle.exchangeCycleValue=="3"&&cycleArr[0]=="0">selected</#if>>
			                     	 本年
			                      </option>
			                      <option value="1" <#if changeItemCycle.exchangeCycleValue=="3"&&cycleArr[0]=="1">selected</#if>>
			                  	          下年
			                      </option>
			                      </select>
			                      <select id="y_month" name="y_month" style="width:60px">
			                       <#list 1..12 as temp>
								 	<option value="${temp}" <#if changeItemCycle.exchangeCycleValue=="3"&&cycleArr[1]==(temp+"")>selected</#if>>${temp}</option>
								 </#list>
		                     	 </select>
		                     	 月
		                     	 <select id="y_day" name="y_day" style="width:60px">
			                     <#list 1..31 as temp>
		   						  <option value="${temp}" <#if changeItemCycle.exchangeCycleValue=="3"&&cycleArr[2]==(temp+"")>selected</#if>>${temp}</option>
								 </#list>
		                     	 </select>
		                     	 日执行
		                      </span>
	...
	<script type="text/javascript"> 
		$(document).ready(function(){
			...
			var dataSource = {};
			dataSource.id = '${datSource.id?default('')}';
