  var $jstree = null;
	...
	$(function () {
     	...
     	$jstree = $("#dataSourceTree").bind('click.jstree', function(event){
		...
		.bind('loaded.jstree', function(event){
			var selected = $('#dataSourceTree').jstree("get_selected");
			//如果没选择的节点，默认选择根
			if(selected.size()==0){
				var $root = $('#dataSourceTree li[rel="root"]');//根节点
			    var rt=	$('#dataSourceTree').jstree("select_node",$root);
			}
		})
		.jstree({
			"core" : { "animation" : 250 },
			...
			"json_data" : {
				"ajax" : {
					"url" : path+"/cloudnode/datasource/datasource!dataSourceTree.action",
					"data" : function (n) { 
