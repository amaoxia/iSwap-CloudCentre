...
<body>
		<div class="loayt_01 rlist100">
          <div class="loayt_mian" >
           <table class="main_drop">
       				 <tr>
          <td align="right">
          		<form name="pageForm" method="post"  action="${path}/cloudstorage/apply/apply!listMetaAuth.action">
            	指标名称：	<input name="conditions[e.metaData.targetName,string,like]" value="<#if serchMap.e_metaData_targetName?exists>${serchMap.e_metaData_targetName?default("")}</#if>" onkeypress="showKeyPress()" onpaste="return false" type="text"  />
      ...
      <table class="tabs1">
        <tr>
          ...
          <th>申请部门</th>
          <th>状态</th>
          <th>创建时间&nbsp;<a href="javascript:void(0)" onclick="setOrder(this)" name="e.filedApplyDate"><img src="${path}/images/tabs_arr.png" align="absmiddle" /></a></th>
          <th>操作</th>
        </tr>
        <#list listDatas as entity>
          ...
          <td>
          <a href="javascript:void(0)" class="tabs1_cz" onclick="opdg('${path}/cloudstorage/apply/apply!authView.action?id=${entity.id}','授权信息','450','430')"> 
		  ...
		  <a href="javascript:void(0)" class="tabs1_cz"  <#if entity.filedApplyType=="1">onclick="downLoad('${entity.id}')"<#else>style="color:#ccc;"</#if>><img src="${path}/images/small9/down.gif" />下载申请文件</a>          </td>
           </tr>
        </#list>
       </table>
