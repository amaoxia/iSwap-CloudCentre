...
<head>
		...
		<body>		
							<!--<input type="hidden" ids="ids" value="${tids?default('')}"/>-->
		...
		<link rel="stylesheet" href="${path}/css/zTreeStyle/zTreeIcons.css" type="text/css">
			<script type="text/javascript">
			    ...
			    setting={
			    async : true,//异步加载 
			    asyncUrl: "${path}/cloudstorage/tableinfo/tableinfo!tree.action?itemId=${itemId?default('')}&deptId=${deptId?default('')}&appId=${appId?default('')}",//数据文件 
			    showLine:true,
			    checkable:true,
			    isSimpleData:true,
			    treeNodeKey:"id",
