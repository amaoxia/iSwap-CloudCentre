				      ...
				      <table class="tabs1" style="margin-top:0px;">
				        <tr>
				         <th width="20"><a href="javascript:void(0);" onclick="checkedAll(this,'ids');return false;"></a></th>
				          ...
				          <td><input name="ids" type="checkbox" value="${entity.id}" <#if entity.sourceCode="SYSTEM">disabled="true"</#if>/></td>
				          <td width="28">${entity_index+1}</td>
				          <td><a href="javascript:void(0)" onclick="opdg('${path}/cloudstorage/mongodb/mongodb!view.action?id=${entity.id}','数据源信息查看','550','350');">${entity.sourceName?default('')}</a></td>
				          <td>${entity.sourceCode?default('')}</td>
				          <td>${entity.ip?default('')}</td>
				          <td>${entity.createDate?default('')}</td>
				          <td width="250">
				          <a id="hz2" href="javascript:void(0);" onclick="opdg('${path}/cloudstorage/mongodb/mongodb!updateView.action?id='+${entity.id},'数据源编辑','650','550');" class="tabs1_cz">
				          <img src="${path}/images/czimg_edit.gif" />编辑</a>&nbsp;
				          <a id="hz4" href="javascript:void(0);" onclick="del(${entity.id});" class="tabs1_cz" <#if entity.sourceCode="SYSTEM">disabled="true" style="cursor:default;"</#if>>
				          <img src="${path}/images/small9/czimg_sc.gif" <#if entity.sourceCode="SYSTEM">disabled="true"</#if>/>删除</a>&nbsp;
				       ...
				       <div class="tabs_foot"> 
					       <span class="tfl_btns">
					      	...
					      	<a href="javascript:void(0)" onclick="delMany('${path}/cloudstorage/mongodb/mongodb!delete.action');" class="tfl_blink"><b class="hot">删除</b></a>
					      </span> 
		...
		<script> 
			...
			function del(id){
				if (confirm("确定要删除选中记录吗？")) {
					document.forms['pageForm'].action ="${path}/cloudstorage/mongodb/mongodb!delete.action?ids="+id ;
			...
			function  testConn(connId){
				$.ajax({
			  		   type: "POST",
					   url: "${path}/cloudstorage/mongodb/mongodb!testConnDataSource.action",
					   data: "id=" + connId,
