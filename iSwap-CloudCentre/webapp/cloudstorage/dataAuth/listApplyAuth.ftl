...
<body>
          ...
          <div class="loayt_mian" >
           <table class="main_drop">
       				 <tr>
          <td align="right">
         	 <form method="post" name="pageForm" id="pageForm" action="${path}/cloudstorage/dataAuth/dataAuth!listApplyAuth.action?type=${type?default('')}">
       		     指标名称：<input type="text" onkeypress="showKeyPress()" onpaste="return false" name="targetName" value="${targetName?default('')}"/>
      ...
      <table class="tabs1">
        ...
        <#list listDatas as entity>
          ...
          <td><#if entity.dataAuthState?exists><#if entity.dataAuthState=='0'>未授权<#elseif entity.dataAuthState=='2'>修改授权<#else>授权</#if><#else>未授权</#if></td>
          <td>
          <a href="javascript:void(0)" class="tabs1_cz" onclick="opdg('${path}/cloudstorage/dataAuth/dataAuth!authView.action?id=${entity.id}','申请函信息','450','430')"> 
          ...
          <#if entity.dataApplyType=="1"><a href="javascript:void(0)"  onclick="downLoad('${entity.id}')" class="tabs1_cz">
