		 ...
		 <form name="saveForm" action="${path}/cloudstorage/metaDataMapper/metaDataMapper!update.action" method="post" id="saveForm">	
			...
			<table  style="margin-top:2px;">
		        ...
		        <tr>
				  ...
				  <td width="30%"><input type="text" id="mapName" name="mapper.mapName" value="${mapper.mapName}" disabled="disabled"/></td>
				  <td width="20%">mapper编码：</td>
				  <td width="30%"><input type="text" id="mapCode" value="${mapper.mapCode}" readonly="readonly" disabled="disabled"/></td>
		        </tr>
	        </table>				
			...
			$.post(url+'?currTime='+new Date(),{itemId:itemId,metaDataId:itemId},function(result, status){
				...
				if(result.success){
					...
					if(srcbool && tarbool){
						...
						$('#mappingTbody select').attr("disabled",true);
					}	
				}	
			});
		}
